...
const specialRegex = [
  /(\d\.)?\d+(×|倍率)/,
  /IPLC|IEPL|BGP|Kern|Edge|Pro|Std|Exp|Biz|Fam|商宽|家宽|Game|Buy|Zx|LB|CF|UDP|GPT|UDPN/i,
];
const nameclear =
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 预处理 防止预判或遗漏
    Object.keys(rurekey).forEach((rk) => {
      if (rurekey[rk].test(e.name)) {
        e.name = e.name.replace(rurekey[rk], rk);
      if (BLKEY) {
        bktf = true
        let BLKEY_REPLACE = "",
        re = false;
    ...
    if (findKey?.[1]) {
      ...
      keyover = keyover
        .concat(firstName, usflag, nNames, findKeyValue, ikeys, retainKey, ikey)
        .filter((k) => k !== "");
